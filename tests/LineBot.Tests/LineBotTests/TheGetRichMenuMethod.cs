    ...
    public partial class LineBotTests
    {
        ...
        public class TheGetRichMenuMethod
        {
            ...
            public async Task ShouldReturnNullWhenResponseContainsWhitespace()
            {
                ...
                var bot = TestConfiguration.CreateBot(httpClient);
                var richMenuResponse = await bot.GetRichMenu("test");

                Assert.IsNull(richMenuResponse);
            }
            ...
            public async Task ShouldCallTheCorrectApi()
            {
                ...
                var bot = TestConfiguration.CreateBot(httpClient);
                var richMenuResponse = await bot.GetRichMenu(richMenuId);
            ...
            [TestMethod]
            [DeploymentItem(JsonDocuments.RichMenuResponse)]
            public async Task ShouldReturnRichMenuResponseWhenResponseIsCorrect()
            {
                var richMenuId = "f22df647-b12f-427c-85c5-8238bee6bb45";

                var httpClient = TestHttpClient.Create(JsonDocuments.RichMenuResponse);
                var bot = TestConfiguration.CreateBot(httpClient);
                var richMenuResponse = await bot.GetRichMenu(richMenuId);

                Assert.IsNotNull(richMenuResponse);
                Assert.AreEqual("f22df647-b12f-427c-85c5-8238bee6bb45", richMenuResponse.Id);
                Assert.IsFalse(richMenuResponse.Selected);

                var size = richMenuResponse.Size;
                Assert.IsNotNull(size);
                Assert.AreEqual(2500, size.Width);
                Assert.AreEqual(1686, size.Height);

                Assert.IsNotNull(richMenuResponse.Areas);
                var area = richMenuResponse.Areas.FirstOrDefault();
